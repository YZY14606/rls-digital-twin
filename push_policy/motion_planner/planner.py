...
class Planner:
    ...
    def IK(self, goal_pose, start_qpos, mask=None, n_init_qpos=800, threshold=1e-3):
        """
        Inverse kinematics
    ...
    def plan_qpos_to_pose(
        ...
        # # Filter the IK solution (yzy)
        def get_max_three_margin_goal_qpos(goal_qpos_copy,qpos_limit):
            min_distances = []
            lower = qpos_limit[:, 0]
            upper = qpos_limit[:, 1]
            for arr in goal_qpos_copy:
                # 计算与下限和上限的距离
                dist_to_lower = np.abs(arr - lower)
                dist_to_upper = np.abs(arr - upper)
                # 取两者中的最小值
                dist = np.minimum(dist_to_lower, dist_to_upper)
                dist_single_value = np.min(dist[:7])
                min_distances.append(dist_single_value)
            # 找到 min_distances 中最小的2个值的索引
            max_indices = np.argpartition(min_distances, -3)[-3:]  # 最大的2个索引
            # 从 goal_qpos_copy 中取出对应的两个元素
            selected_elements = [goal_qpos_copy[i] for i in max_indices]
            return selected_elements

        if len(goal_qpos) > 3:
            goal_qpos = get_max_three_margin_goal_qpos(goal_qpos_copy = goal_qpos.copy(), qpos_limit = self.joint_limits)
    ...
    def plan_screw(
        ...
        if theta < -1e4:
            print("The theta get wrong value.")
        ...
        while True:
            ...
            if np.linalg.norm(delta_twist) < 1e-4 or collide or not within_joint_limit:
                print(f"delta_twist is wrong: {np.linalg.norm(delta_twist) < 1e-4}")
                print(f"The joint limit error: {not within_joint_limit}.")
                print(f"The robot collides with obstacle: {collide}.")
                return {"status": "screw plan failed"}
    ...
    def yzy_plan_qpos_to_qpos(
        ...
        if status == "Exact solution":
            ...
            return {"status": "RRT Failed. %s" % status}, np.array([0,0,0])
    ...
    def plan_linear_motion(
        ...
        constraint_tolerance=1e-3,
        verbose=False,
        interpolatio_time = 10
    ):
        """
        This is created by myself (yzy).
        plan from a start configuration to a goal pose of the end-effector
        ...
        if wrt_world:
            goal_pose = self.transform_goal_to_wrt_base(goal_pose)
            current_pose = self.transform_goal_to_wrt_base(current_ee_pose)

        # Get the position
        p1 = current_pose[:3]  # [x1, y1, z1]
        p2 = goal_pose[:3]  # [x2, y2, z2]
        interpolated_points = np.linspace(p1, p2, num=interpolatio_time)
        # Get the next goal poses
        goal_poses = np.zeros((interpolatio_time -1,7), dtype = float)
        goal_poses[:,:3] = interpolated_points[1:,:]
        goal_poses[:,3:7] = goal_pose[3:7]

        all_path = []
        for i in range(goal_poses.shape[0]-1):
            goal_pose = goal_poses[i,:].reshape(-1,1)
            self.robot.set_qpos(current_qpos, True)
            ...
            status, path = self.yzy_plan_qpos_to_qpos(
                goal_qpos,
                current_qpos,
                time_step,
                rrt_range,
            ...
            current_qpos = np.concatenate([path[-1],np.array([0,0])],axis=0)
